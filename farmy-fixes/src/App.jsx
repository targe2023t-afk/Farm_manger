...
export default function App() {
  ...
  const syncToServer = useCallback(async()=>{
    ...
    } catch(_){ setSyncStatus("error"); showToast("📴 لا يوجد اتصال - البيانات محفوظة محلياً"); }
  ...
  const NAV = [
    ...
    {k:"inventory",ic:Nav.inv,l:"المخزون"},
  ...
  return (
    ...
    <div className="app" dir="rtl">
      ...
      {syncStatus==="error"  &&<div className="sync-bar" style={{background:"#ffebee",color:"var(--red)"}}>⚠️ تعذر الاتصال - محفوظ محلياً</div>}
      ...
      {page==="dashboard"
        ...
        : <div className="page-top">
            ...
            <div className="page-title">{NAV.find(n=>n.k===page)?.l||page}</div>
            {/* logout in sub-pages */}
      ...
      <div className="bnav">
        ...
        <button className={`bni ${["reports","users"].includes(page)?"on":""}`} onClick={()=>setPage("reports")}>
          {Nav.rep}<span>التقارير</span>{["reports","users"].includes(page)&&<div className="bni-dot"/>}
        </button>
      </div>
...
function DashPage({expenses,revenues,inventory,workers,lowStock,setPage}) {
  ...
  return (
    <div>
      ...
      {lowStock.length>0&&(
        <div className="section">
          <div className="section-title">تنبيهات المخزون</div>
...
function InvPage({data,setData,trash,setTrash,usageLog,setUsageLog,canEdit,canDel,showToast,lowStock,audit}) {
  ...
  const doUse=()=>{
    ...
    setUse(null);setUQ("");showToast("تم الخصم من المخزون ✓");
  };
  ...
  const handleScan=(code)=>{
    ...
    if(found){ setQ(code); showToast(`✅ تم العثور على: ${found.name}`); }
    else{
    ...
    <div>
      ...
      {filtered.map(item=>{
        ...
        return (
          <div key={item.id} className="inv-item">
            ...
            {(out||low)&&<div style={{marginTop:8,background:"#ffebee",borderRadius:8,padding:"6px 10px",fontSize:12,color:"var(--red)",fontWeight:700}}>⚠️ {out?"نفد المخزون":"كمية منخفضة"}</div>}
          </div>
        );
      })}
      ...
      {useItem&&(
        <div className="modal-ov" onClick={()=>setUse(null)}>
          <div className="modal-box" onClick={e=>e.stopPropagation()}>
            <div className="modal-handle"/>
            <div className="modal-title">استخدام من المخزون — {useItem.name}</div>
...
function RepPage({expenses,revenues,workers,inventory,auditLog,users}) {
  ...
  const REP_TABS=[{k:"financial",l:"مالي"},{k:"workers",l:"العمالة"},{k:"inventory",l:"المخزون"},{k:"audit",l:"سجل التغييرات"}];
  ...
  return (
    <div>
      ...
      {repTab==="inventory"&&(
        <div className="section" style={{marginTop:12}}>
          <div className="section-title">قيمة المخزون الحالي</div>
          <div className="summary-card" style={{marginBottom:14}}>
            <div className="sum-row"><span className="sum-label">📦 إجمالي قيمة المخزون</span><span className="sum-value g">{fmt(invValue)} جنيه</span></div>
...
function UsrPage({users,setUsers,currentUser,showToast,audit}) {
  ...
  const [f,setF]=useState({});
  const s=(k,v)=>setF(x=>({...x,[k]:v}));
  const pageLabels={dashboard:"الرئيسية",expenses:"المصروفات",revenue:"الإيرادات",inventory:"المخزون",workers:"العمالة",reports:"التقارير"};
