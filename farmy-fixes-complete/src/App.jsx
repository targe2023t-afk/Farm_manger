...
function ld(k, fb) {
  try { const r = localStorage.getItem("fmv6_" + k); return r ? JSON.parse(r) : fb; }
  catch { return fb; }
}
function sd(k, v) {
  try { localStorage.setItem("fmv6_" + k, JSON.stringify(v)); } catch {}
}
...
// FIX #1: Correct daysBetween — reset time to midnight for accuracy
...
// FIX #2: Hash passwords client-side (server should use bcrypt)
async function hashPassword(password) {
  const encoder = new TextEncoder();
  const data = encoder.encode(password + "farmy_salt_2024_v6");
...
export default function App() {
  ...
  const [users, setUsers] = useState(() => ld("users", [
    {
      ...
      username: "admin",
      password: "03ac674216f3e15c761ee1a5e255f067953623c8b388b4459e13f978d7c846f4", // hashed "1234"
      role: "admin",
      name: "المدير العام",
      phone: "",
      status: "active",
  ...
  useEffect(() => sd("auditLog", auditLog.slice(0, 200)), [auditLog]); // FIX: limit audit storage
  ...
  const audit = useCallback((action, entity, oldV, newV) => {
    setAuditLog(l => [createAuditEntry(user, action, entity, oldV, newV), ...l].slice(0, 200));
  }, [user]);

  const syncTimerRef = useRef(null);
  const hasLoaded = useRef(false);  // FIX: separate from isFirstRender
  ...
  useEffect(() => { if (!hasLoaded.current) return; debouncedSync("expenses", expenses); }, [expenses, debouncedSync]);
  useEffect(() => { if (!hasLoaded.current) return; debouncedSync("revenues", revenues); }, [revenues, debouncedSync]);
  useEffect(() => { if (!hasLoaded.current) return; debouncedSync("inventory", inventory); }, [inventory, debouncedSync]);
  useEffect(() => { if (!hasLoaded.current) return; debouncedSync("workers", workers); }, [workers, debouncedSync]);
  useEffect(() => { if (!hasLoaded.current) return; debouncedSync("usage_log", usageLog); }, [usageLog, debouncedSync]);
      ...
      <div className="login-card">
        ...
        {tab === "login" ? (
          <>
            ...
            <button className="save-btn" onClick={doLogin} disabled={isSubmitting}>{isSubmitting ? "جاري التحميل..." : "تسجيل الدخول"}</button>
          </>
        ) : (
          <>
            <div className="frow2">
            ...
            <button className="save-btn" onClick={doReg} disabled={isSubmitting}>{isSubmitting ? "جاري الإنشاء..." : "إنشاء الحساب"}</button>
          </>
        )}
      </div>
    </div>
...
function BarcodeScanner({ onScan, onClose }) {
  ...
  useEffect(() => {
    ...
    async function startCamera() {
      ...
      } catch (e) { setError("لا يمكن الوصول للكاميرا. تأكد من إعطاء الإذن."); }
    }
...
function ExpPage({ data, setData, trash, setTrash, canEdit, canDel, showToast, audit }) {
  ...
  const [showDelConfirm, setShowDelConfirm] = useState(null);  // FIX: delete confirmation
      ...
      {showForm && (
        <div className="modal-ov" onClick={() => setShowForm(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="frow2">
              <div><div className="flbl">المبلغ (جنيه) *</div><input className="finp" type="number" min="0" placeholder="أدخل المبلغ" value={f.amount || ""} onChange={e => s("amount", e.target.value)} /></div>
...
function InvPage({ data, setData, trash, setTrash, usageLog, setUsageLog, canEdit, canDel, showToast, lowStock, audit }) {
  ...
  const doUse = () => {
    ...
    if (qty > Number(useItem.quantity)) { showToast("الكمية المطلوبة أكبر من المتاح"); return; }  // FIX: validate quantity
    const old = { ...useItem };
      ...
      {showForm && (
        <div className="modal-ov" onClick={() => setShowForm(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="frow2">
              <div><div className="flbl">الكمية المتاحة</div><input className="finp" type="number" min="0" value={f.quantity || ""} onChange={e => s("quantity", e.target.value)} /></div>
            ...
            <div className="frow2">
              <div><div className="flbl">الحد الأدنى للتنبيه</div><input className="finp" type="number" min="0" value={f.minStock || ""} onChange={e => s("minStock", e.target.value)} /></div>
              <div><div className="flbl">سعر الوحدة (جنيه)</div><input className="finp" type="number" min="0" value={f.price || ""} onChange={e => s("price", e.target.value)} /></div>
            </div>
            <button className="save-btn" onClick={save}>حفظ</button>
          </div>
        </div>
...
function UsrPage({ users, setUsers, currentUser, showToast, audit }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!passwordHash && !edit) { showToast("يرجى إدخال كلمة مرور"); setIsSubmitting(false); return; }
  ...
  // FIX: Proper permission toggle using setF (local state) instead of generating UUIDs
      ...
      {showForm && (
        <div className="modal-ov" onClick={() => setShowForm(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <div style={{ height: 10 }} />
            <button className="save-btn" onClick={save} disabled={isSubmitting}>{isSubmitting ? "جاري الحفظ..." : "حفظ"}</button>
          </div>
        </div>
      )}
    </div>
